...
Similary, suppose the Republician win the election with $P(y)$. It
doesn't affect my odd to get an A, and to make both happen, $P(x,y)=P(y)P(x)$.

Intuitively, the rule holds because with $P(x)$, $x$happens, but
it doesn't affect the odd that $y$happens, so I need to multiply
$P(y)$.\\
\\
...
E(x^{2}) & = & -\frac{\sigma}{\sqrt{2\pi}}\int_{-\infty}^{\infty}x\left[\exp\left(-\frac{x^{2}}{2\sigma^{2}}\right)\right]'dx\\
 & = & -\frac{\sigma}{\sqrt{2\pi}}\left[\exp\left(-\frac{x^{2}}{2\sigma^{2}}\right)\bigg|_{-\infty}^{\infty}-\int_{-\infty}^{\infty}\exp\left(-\frac{x^{2}}{2\sigma^{2}}\right)dx\right]\\
...
Since $g_{i}$ has M elements, the maximum dimension of $G$ spanned
by $\{g_{i},i=1...N\}$ is M. Therefore,

\begin{eqnarray*}
dim\left(R(G)\right) & \leq & M<N=dim(R^{N})
\end{eqnarray*}
...
The diagonal components of $\boldsymbol{H}$


\subsection*{(c) 5 points}

\begin{eqnarray*}
G^{T}d & = & (g_{1}^{T}d,...,g_{M}^{T}d)^{T}
\end{eqnarray*}


It shows the projection of $d$ into the model space.


\subsection*{(d) 5 points}

This equation implies that $Gm$ is the projection of $d$ in the
model space. Thus, the least-squares solution $m$ is the coordinate
of $d$ in the model space.
\end{document}
