...
\section{Problem 1 (graded by Kangchen) - 24+1 points}
...
\section{Problem 3 (graded by Kangchen) - 24+1 points }
...
red lines mark the four roots. We also plot the function $f(x)$ in black.\\
...
enough to a given root, Newton's method will generally find this root that is
closest to the initial guess, unless the initial guess is close to an extreme point. \\
If the initial guess is far away from the roots or close to an extreme point, there is no guarantee
that Newton's method will find the closest root. The jump in x is controlled by the gradient at the current point. If the current
