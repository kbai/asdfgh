...
where $c$ is a constant and is determined by
...
P(d_{i}|L) & = & \frac{3}{4}\frac{1}{\sigma\sqrt{2\pi}}\exp\left(-\frac{(d_{i}-L)^{2}}{2\sigma^{2}}\right)+\frac{1}{4}\frac{1}{\sigma\sqrt{2\pi}}\exp\left(-\frac{(d_{i}-(L+1))^{2}}{2\sigma^{2}}\right)\\
 ...
 & = & \frac{3}{4}\mathcal{N}(d_{i},\sigma)+\frac{1}{4}\mathcal{N}(d_{i}-1,\sigma)
\end{eqnarray*}
...
We see that the best estimate of $L$ is $8.2$ cm. It's not an approximate
number, since the accuracy is to $0.01$ cm.
...
\[
P(\boldsymbol{m}|d)\propto P(d|\boldsymbol{m})P(\boldsymbol{m})
\]
...
\normalsize\tiny
\lstinputlisting{./HW3_Q1/compute_misfit.m}
\normalsize\tiny
