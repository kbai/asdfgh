...
likelihood $P(\{d_{k}\}|\boldsymbol{m})$ by a prior for model $P(\boldsymbol{m})$.
In this case we will use a gaussian distribution for model parameter
$p$ with $\mu=35$ and $\sigma=6$.
...
\begin{eqnarray*}
P(\boldsymbol{m}|\{d_{k}\})=e^{-F(\boldsymbol{m})}\frac{1}{p}
\end{eqnarray*}
...
experiment is not affected by prior information. Suppose we have some
...
\end{align}
Because $\exp$ and $\ln$ are monotonic functions, we therefore need
to find the minimum of the sum of the equared error (least-squares)
\begin{equation}
...
\begin{equation}
p(\mu|x)\propto e^{-F(\mu)}\approx e^{-F(\mu_{0})-\frac{1}{2}F''(\mu_{0})(\mu-\mu_{0})^{2}},
\end{equation}
...
then $Q=\begin{bmatrix}cos(\theta) & -sin(\theta)\\
sin(\theta) & cos(\theta)
...
deadling with an integral of the form 
\begin{align}
\sigma_{x}^{2} & =\frac{\int dxdy\,x^{2}\exp\left(-\frac{1}{2}x^{T}Ax\right)}{\int dxdy\,\exp\left(-\frac{1}{2}x^{T}Ax\right)}\\
 & =\frac{\int dxdy\,x^{2}\exp\left(-\frac{1}{2}(Ax^{2}+2Cxy+By^{2})\right)}{\int dxdy\,\exp\left(-\frac{1}{2}(Ax^{2}+2Cxy+By^{2})\right)}\\
 & =\frac{\int dxdy\,x^{2}\exp\left(-\frac{1}{2}(A-C^{2}/B)x^{2}-\frac{1}{2}B(y+Cx/B)^{2}\right)}{\int dxdy\,\exp\left(-\frac{1}{2}(A-C^{2}/B)x^{2}-\frac{1}{2}B(y+Cx/B)^{2}\right)}\\
 & =\frac{\int dx\,x^{2}\exp\left(-\frac{1}{2}(A-C^{2}/B)x^{2}\right)}{\int dx\,\exp\left(-\frac{1}{2}(A-C^{2}/B)x^{2}\right)}
\end{align}
